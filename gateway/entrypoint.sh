...
#   - Dev local: se monta el archivo físico desde infra/certs/ y su ruta se
#     inyecta en JWT_PUBLIC_KEY_FILE; aquí se carga su contenido en
...
if [ -n "${JWT_PUBLIC_KEY_FILE:-}" ]; then
  if [ ! -f "$JWT_PUBLIC_KEY_FILE" ]; then
    echo "entrypoint: JWT_PUBLIC_KEY_FILE='$JWT_PUBLIC_KEY_FILE' no existe (¿montaste infra/.certs?)" >&2
    exit 1
  fi
